...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Neo4j-5-green" alt="Neo4j">
...
为控制 LLM 成本，系统不预先对所有文档做实体抽取，而是基于检索命中频率按需触发：

```
用户查询 → 检索命中父块 → 更新 hit_count
                              │
                    hit_count ≥ 3 ?
                    ┌─── YES ───┴─── NO ───┐
                    ▼                       ▼
...
- **Entity 节点**：`(name, type, user_id)` 唯一约束，MERGE 语义跨文档去重
- **ParentChunk 节点**：轻量引用（id, filename, heading_path），实际内容在 PostgreSQL
- **关系**：`RELATES_TO`（实体间）+ `MENTIONED_IN`（实体→父块）
- **图谱检索**：LLM 抽取查询实体 → Neo4j 1-2 跳遍历 → 返回关联父块 ID
...
- 多子查询并行检索 + 三路 RRF 融合（向量 + BM25 + 知识图谱）
- Redis 缓存检索结果（相同查询在 TTL 内直接命中）

### 🕸️ 知识图谱
- Neo4j 图数据库存储实体与关系，支持跨文档多跳推理
- 按需抽取：仅对命中 ≥3 次的高频父块触发 LLM 实体抽取，控制成本
- 开放 Schema：不限定实体类型，LLM 自由抽取文本中的关键实体和关系
- 图谱检索作为第三路融合路径，1-2 跳图遍历关联实体对应的文档块
- 前端知识图谱浏览页面：实体列表、搜索、关系详情、关联文档溯源
- 支持手动触发全量文档抽取（绕过阈值）
...
启动 PostgreSQL (:5433)、Redis (:6380)、Milvus (:19530)、Neo4j (:7687)、etcd、MinIO 共 6 个服务。
...
│   │   ├── graph_service.py       # Neo4j 知识图谱 CRUD
...
│       │   ├── DocumentsPage.tsx   # 文档管理 + 分块预览 + 图谱抽取
│       │   ├── KGPage.tsx         # 知识图谱浏览（实体/关系/溯源）
│       │   └── LoginPage.tsx       # 注册/登录
...
| **登录 / 注册** | JWT 认证，防用户枚举 |
| **智能问答** | 流式回答，检索过程可视化 |
| **文档管理** | 上传/删除/列表，分层分块预览，一键加入图谱 |
| **知识图谱** | 实体列表/搜索/详情，关系可视化，文档溯源 |
