...
# 测试（使用 testcontainers 启动 PostgreSQL，需要 Docker 运行）
...
**分块策略**（`hierarchical_chunker.py`）：H1/H2 标题触发父块边界，H3 及以下留在当前父块内。叶子块通过 RecursiveCharacterTextSplitter 切分（300 字，30 字重叠）。表格和代码块原子保留。仅含标题无正文的父块被跳过。不足 100 字的叶子块合并到相邻块。
